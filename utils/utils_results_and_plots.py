...
def get_selection_technique_rankings(dataset_name, dataset_results, dataset_type, task=None):
    ...
    for method, method_results in dataset_results.items():
        if dataset_type == 'real_world':
            if 'models_performance' in method_results:
                method_scores[method] = method_results['models_performance']['catboost']
        else:  # synthetic
    ...
    reverse_sort = True
    if dataset_type == 'real_world' and task == 'regression':
        reverse_sort = False
    
    # Sort and assign rankings
    sorted_methods = sorted(method_scores.items(), key=lambda x: x[1], reverse=reverse_sort)
    rankings = {}
    current_rank = 1
    prev_score = None
    skip_ranks = 0
...
def get_rankings(real_world_results, synthetic_results, real_world_metadata):
    """
    ...
    for dataset_name, dataset_results in real_world_results.items():
        ...
        rankings['real_world'][dataset_name] = get_selection_technique_rankings(
            dataset_name, dataset_results, 'real_world', task
        )
...
def calculate_mean_metrics_overall(rankings, real_world_results, synthetic_results):
    """
    ...
    # Real-world means
    method_metrics = defaultdict(lambda: {'total_time': 0, 'total_features': 0, 'total_rank': 0, 'count': 0})
    ...
    for dataset_name, dataset_results in real_world_results.items():
        for method, method_results in dataset_results.items():
            ...
            method_metrics[method]['total_rank'] += rankings['real_world'][dataset_name][method]
    ...
    for method, metrics in method_metrics.items():
        ...
        mean_metrics['real_world'][method] = {
            ...
            'mean_model_scores_ranking': metrics['total_rank'] / count,
    ...
    for dataset_name, dataset_results in results.items():
        ...
        for method, method_results in dataset_results.items():
            ...
            if dataset_type == 'real_world':
                ...
                metrics['model_scores_ranking']['total'] += rankings[method]
                metrics['model_scores_ranking']['count'] += 1
            else:
...
def create_real_world_plot(results, METHOD_ORDER, FAMILY_ORDER, METHOD_FAMILIES, FAMILY_COLORS, METHOD_NAMES, title_suffix="", output_suffix=""):
    ...
    for method_name, order_num in METHOD_ORDER.items():
        if method_name in results:
            ...
            methods_data.append({
                ...
                'ranking': metrics['mean_model_scores_ranking'],
    ...
    for data in sorted(methods_data, key=lambda x: x['index']):
        ...
        # Size calculation for execution time visualization
        ...
        # Plot point
        ax.scatter(data['ranking'], data['features'], s=size, c=color, alpha=0.6)
        ax.text(data['ranking'], data['features'], str(data['index']),
    ...
    # Customize plot
    ax.set_xlabel('Average Model Scores ranking\n(lower is better)', fontsize=14)  # Increase fontsize
    ax.set_ylabel('Average number of selected features', fontsize=14)  # Increase fontsize
    ax.set_title(f'Average results in real-world datasets{title_suffix}', pad=20, fontsize=16)  # Increase fontsize
...
def create_synthetic_plot(results, METHOD_ORDER, FAMILY_ORDER, METHOD_FAMILIES, FAMILY_COLORS, METHOD_NAMES, title_suffix="", output_suffix=""):
    ...
    ax.set_ylabel('Average execution time (minutes)', fontsize=14)  # Increase fontsize
    ax.set_title(f'Average results in synthetic datasets{title_suffix}', pad=20, fontsize=16)  # Increase fontsize
    ax.grid(True, linestyle='--', alpha=0.7)
    ax.set_xlim(0, 20)
    ax.set_yscale('log')
